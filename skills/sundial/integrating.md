...
sundial add poll \
  ...
  --command 'codex exec resume <thread> --model <chosen-model> -c model_reasoning_effort="low" --dangerously-bypass-approvals-and-sandbox "a reply arrived; continue the campaign" >>~/.config/my-tool/logs/reply-callback.log 2>&1' \
...
- Codex scheduled runs: use plain output redirected to a local log, set model and reasoning explicitly, and keep prompts short. Example: `codex exec --model <chosen-model> -c model_reasoning_effort="low" --dangerously-bypass-approvals-and-sandbox "Run <skill-name> now." >>/path/to/run.log 2>&1`.
- Codex `--json`: use only for short bootstrap/id-capture calls where structured events are required. Do not use it as the default recurring scheduled-run pattern; image/tool payloads can make stdout large and brittle.
...
**Codex** — for bootstrap only, NDJSON on stdout. First line is `{"type":"thread.started","thread_id":"..."}`:
...
sundial add at --at "2026-04-24T10:00:00" \
  --command "codex exec resume $thread_id --model <chosen-model> -c model_reasoning_effort=\"low\" --dangerously-bypass-approvals-and-sandbox 'continue where we left off' >>~/.config/my-tool/logs/resume-$thread_id.log 2>&1" \
