...
skills/              → top-level embedded SKILL.md tree (sundial/SKILL.md, setup.md, scheduling.md). embed.go exposes skills.FS.
internal/
...
2. **Consumers of sundial** (agents scheduling events, engineers building tools on top) — the skill tree at `skills/sundial/`. `SKILL.md` is a catalog that signposts to `scheduling.md` (the full consumer guide: scheduling repertoire for agent users plus a closing section for agent-adjacent tool builders), with `setup.md` as the one-time-init prerequisite. Changes to what consumers see happen there, not in CLAUDE.md or README.md.
